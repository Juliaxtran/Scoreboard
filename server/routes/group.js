...
module.exports = (db, dbQueries) => {
  ...
  router.post('/add/:group_id', (req, res) => {
    ...
    dbQueries.getUserByEmail(email, db).then((player) => {
      if (player) {
        console.log('player info', player);
        dbQueries.addPlayerToGroup(group_id, player.id, db).then((group) => {
          if (group) {
            res.status(200).send({
              success: true,
            ...
            res.status(400).send({
              success: false,
              message:
                'Player not added to group, player needs to sign up first',
            });
          }
        });
      } else {
