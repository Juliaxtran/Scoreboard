...
const LeaderBoard = () => {
  ...
  useEffect(() => {
    axios
      .get(`/group/leaderboard/${group_id}`, { withCredentials: true })
      .then((res) => {
        const leaderboard = Array.isArray(res.data.leaderboard)
      ...
      >
        <Paper elevation={0}>
          ...
          >
            ...
            >
              {matches && matches.length === 0 ? "Players" : "Leaderboard"}
            </h1>
          </Link>
          <Box
            sx={{
          ...
          >
            ...
            {matches && matches.length > 0 &&
              ...
              leaderboard.slice(0, 6).map((player, index) => {
                ...
                return (
                  <div className="leader-box" key={player.id}>
                    <h1>{index + 1}</h1>
                    {/* Avatar icon */}
                    <Avatar
                      src={trophyImage}
                    ...
                    <div className="leader-info">
                      ...
                      <p>
                        ...
                        {Number(player.win_ratio).toFixed(1)}
                      </p>
                    </div>
                  </div>
                );
